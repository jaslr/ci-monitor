...
export async function pollGCPCloudBuild(): Promise<void> {
  console.log('Polling GCP Cloud Build...');

  // Find projects that use GCP Cloud Build
  const gcpProjects = projects.filter((p) =>
    p.services.some((s) => s.provider === 'gcp' && s.category === 'ci')
  );
  ...
  for (const project of gcpProjects) {
    const gcpService = project.services.find(
      (s) => s.provider === 'gcp' && s.category === 'ci'
    );
    if (!gcpService || !gcpService.gcpProjectId) continue;
    ...
    try {
      await db.insertStatusCheck(
        gcpService.id,
        status,
        `Build ${build.status}${duration ? ` (${Math.round(duration / 60)}m)` : ''}`
      );
    } catch (err) {
    ...
    if (status !== prevStatus) {
      ...
      if (status === 'down' && prevStatus !== 'down') {
        await dispatchAlert(
          ...
          gcpService.id,
          'deploy_failure',
          `GCP Cloud Build failed for ${gcpService.gcpProjectId}: ${build.failureInfo?.detail || build.status}`
        );
      }
    }
  }
